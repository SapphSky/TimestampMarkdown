...
const toFormat = (format: string) => {
  ...
  switch (format) {
    ...
    case "R":
      return useTimeAgo(value, { showSecond: true, updateInterval: 1000 });
  }
};

const toMarkdown = () => {
  const timestamp: number = new Date(dateTime.value) / 1000;
  ...
  <div>
    <main>
      <div class="hero bg-base-300 min-h-screen">
        <div class="hero-content text-cen ter">
          <ClientOnly>
            <div class="max-w-md">
              <fieldset class="fieldset">
                ...
                <select v-model="format" class="select select-md">
                  ...
                  <option value="f">
                    Short date & time ({{ toFormat("f") }})
                  </option>
                  <option value="F">
                    Long date & time ({{ toFormat("F") }})
                  </option>
              ...
              <div v-if="dateTime && format" class="py-4">
                ...
                <code
                  ...
                  @click="copyToClipboard(toMarkdown())"
                >
                  {{ toMarkdown() }}
                </code>
              </div>
    ...
    >
      ...
      <nav class="grid-flow-col gap-4 md:place-self-center md:justify-self-end">
        <img
          ...
          src="https://img.shields.io/github/stars/SapphSky/TimestampMarkdown?style=for-the-badge"
        />
        <a href="https://github.com/SapphSky/TimestampMarkdown">
          View on GtiHub
        </a>
        <a href="https://ko-fi.com/sapphsky"> Support on Ko-fi </a>
      </nav>
    </footer>
  </div>
</template>
